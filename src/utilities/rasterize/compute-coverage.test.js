...
eq(computeStrokeCoverage(aaHalf, 0, new Color(0, 0, 0, 0), 'center', 1, 1), 0);
eq(computeStrokeCoverage(aaHalf, 0, new Color(0, 0, 0, 1), 'center', 0, 1), 0);

// Returns 1 inside stroke band (center position).
const strokeColor = new Color(0, 0, 0, 1);
eq(computeStrokeCoverage(aaHalf, 0, strokeColor, 'center', 2, 1), 1);

// Returns falloff outside band within AA region.
eq(computeStrokeCoverage(aaHalf, 1.25, strokeColor, 'center', 2, 1), 0.5);

// Returns 1 inside outside-positioned stroke band.
eq(computeStrokeCoverage(aaHalf, 0.5, strokeColor, 'outside', 2, 1), 1);

// Returns 1 inside inside-positioned stroke band.
eq(computeStrokeCoverage(aaHalf, -0.5, strokeColor, 'inside', 2, 1), 1);

// Returns 0 beyond AA region.
eq(computeStrokeCoverage(aaHalf, 5, strokeColor, 'center', 2, 1), 0);
